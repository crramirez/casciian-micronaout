...
Micronaut 4.x integration that embeds a [Casciian][casciian] TUI into your
Micronaut application and exposes it over SSH **or** over a Unix domain
socket. Drop the jar on the classpath, point an SSH client (or the
...
with full access to your Micronaut beans.

Built for compile-time DI: the integration adds zero reflective lookups
and is GraalVM-native-image friendly out of the box.

## Why

Micronaut gives you a web view for your users. Sometimes you also want a
terminal view for operators: a richer alternative to a REPL or a one-off
admin endpoint.
...
* **Unix domain socket** (default path `/tmp/casciian.sock`) — for
  ...
  application JAR (or native binary) a second time with a single
  argument turns it into a tiny in-container client that attaches to the
  TUI over IPC. No SSH daemon needs to be installed in the image.
...
The same factory bean is used by both the SSH listener and the
...
(e.g. backed by Micronaut Security) to override. The Unix-socket
listener intentionally has no authentication of its own: it relies on
the filesystem permissions of the socket file, which by default
restrict access to the user running the JVM. That matches the threat
model of `docker exec` / `kubectl exec`, where the container runtime
has already authenticated the operator.
...
Over the Unix socket (intended for use inside the container itself,
after `kubectl exec`-ing into the pod):
...
Micronaut entirely and act as a thin terminal client that connects to
the already-running JVM via the configured socket path. The class
behind it is
`io.github.crramirez.casciian.micronaut.client.CasciianConsoleClient` —
wire it into your own `main` like this:
...
the server as framed messages, copies the server's bytes back to
stdout, and polls `stty size` once per second so that resizing the
terminal window is reflected in the TUI.
...
| `casciian.ssh.auto-start`     | `true`                       | Bind the socket automatically on `StartupEvent`. Set to `false` to manage the lifecycle yourself (useful in tests). |
...
| `casciian.unix-socket.auto-start` | `true`                | Bind the socket automatically on `StartupEvent`.                  |
...
Every bean declared by the `@Factory` carries the Micronaut
`@Requires(missingBeans = ...)` guard so you can override any piece:
...
| `CasciianSshServer`                | The whole SSH lifecycle-managed server.        |
| `CasciianUnixSocketServer`         | The Unix-socket lifecycle-managed server.      |
